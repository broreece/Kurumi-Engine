...
public sealed class SaveManager {
    ...
    public int[] GetPartiesSprites(int saveIndex, int maxPartySize, int[] spriteIds) {
        ...
        try {
            string saveInfoFile = Path.Combine(
                saveRootPath,
                $"save_{saveIndex + 1}",
                "info.json"
            );
            string json = File.ReadAllText(saveInfoFile);
            // Load party members and party info.
            PartyData ? partyData = JsonSerializer.Deserialize<PartyData>(json) 
                ?? throw new MissingPartyDataException($"{saveInfoFile} format is broken or missing");
    ...
    public Party LoadParty(PlayableCharacter[] playableCharacters, ItemRegistry itemRegistry, 
        ...
        if (activeSlot == 0) {
            saveInfoFile = Path.Combine(
                saveRootPath,
                "new_game_data",
                "starting_info.json"
            );
            saveInventoryFile = Path.Combine(
                saveRootPath,
                "new_game_data",
                "starting_inventory.json"
            );
        }
        else {
            saveInfoFile = Path.Combine(
            ...
            string saveStatusesFile = Path.Combine(
                saveRootPath,
                $"save_{activeSlot}",
                "statuses.json"
            );
            statusesJson = File.ReadAllText(saveStatusesFile);
        }
        json = File.ReadAllText(saveInfoFile);
        inventoryJson = File.ReadAllText(saveInventoryFile);
    ...
    public EnemyFormationData[] LoadEnemyFormations() {
        ...
        Path.Combine(
            saveRootPath,
            "new_game_data",
            "starting_enemy_formations.json"
        )
        : 
        Path.Combine(
            saveRootPath,
    ...
    public PlayableCharacter[] LoadPlayableCharacters(ICharacterDataLoader characterDataLoader, 
        ...
        if (activeSlot == 0) {
            string newGameRootPath = Path.Combine(
                saveRootPath,
                "new_game_data"
            );
            characterSkillSAbilitiesFile = Path.Combine(
                newGameRootPath,
                "starting_character_skills_abilities.json"
            );
            characterAbilitiesFile = Path.Combine(
                newGameRootPath,
                "starting_character_abilities.json"
            );
            characterStatsFile = Path.Combine(
                newGameRootPath,
                "starting_character_stats.json"
            );
            characterElementsFile = Path.Combine(
                newGameRootPath,
                "starting_character_elements.json"
            );
            characterStatusesFile = Path.Combine(
                newGameRootPath,
                "starting_character_statuses.json"
            );
            characterEquipmentTypesFile = Path.Combine(
                newGameRootPath,
                "starting_character_equipment_types.json"
            );
            characterEquippedFile = Path.Combine(
                newGameRootPath,
                "starting_character_equipped.json"
            );
            characterHpFile = Path.Combine(
                newGameRootPath,
                "starting_character_hp_mp.json"
            );
        }
        else {
            string saveDirectory = Path.Combine(
            ...
            characterSkillSAbilitiesFile = Path.Combine(
                saveDirectory,
                "character_skills_abilities.json"
            );
            characterAbilitiesFile = Path.Combine(
                saveDirectory,
                "character_abilities.json"
            );
            characterStatsFile = Path.Combine(
                saveDirectory,
            ...
            characterStatusesFile = Path.Combine(
                saveDirectory,
                "character_statuses.json"
            );
            characterEquipmentTypesFile = Path.Combine(
                saveDirectory,
                "character_equipment_types.json"
            );
            characterEquippedFile = Path.Combine(
                saveDirectory,
                "character_equipped.json"
            );
            characterHpFile = Path.Combine(
                saveDirectory,
    ...
    private async Task SaveCharactersAsync(ICharacterModifiersAccessor[] playableCharacters, string saveDirectory) {
        ...
        foreach (ICharacterModifiersAccessor character in playableCharacters) {
            ...
            // Stats.
            statsData.Stats.Add([.. character.GetStats()]);
        ...
        // Serialize each file.
        string skillsAbilitiesPath = Path.Combine(
            saveDirectory,
            "character_skills_abilities.json"
        );
        Task saveSkills = SaveJson(skillsAbilitiesData, skillsAbilitiesPath);
        string abilitiesPath = Path.Combine(
            saveDirectory,
            "character_abilities.json"
        );
        Task saveAbilities = SaveJson(baseAbilitiesData, abilitiesPath);
        string statsPath = Path.Combine(
            saveDirectory,
            "character_stats.json"
        );
        Task saveStats = SaveJson(statsData, statsPath);
        string elementsPath = Path.Combine(
            saveDirectory,
            "character_elements.json"
        );
        Task saveElementalResistances = SaveJson(elementResData, elementsPath);
        string statusesPath = Path.Combine(
            saveDirectory,
            "character_statuses.json"
        );
        Task saveStatusResistances = SaveJson(statusResData, statusesPath);
        string equipmentTypesPath = Path.Combine(
            saveDirectory,
            "character_equipment_types.json"
        );
        Task saveEquipmentTypes = SaveJson(equipTypesData, equipmentTypesPath);
        string equipmentPath = Path.Combine(
            saveDirectory,
            "character_equipped.json"
        );
        Task saveEquipment = SaveJson(equippedData, equipmentPath);
        string hpAndMpPath = Path.Combine(
            saveDirectory,
            "character_hp_mp.json"
        );

...
public sealed class EquipmentElementRepository {
    private readonly DatabaseService _databaseService;

    public EquipmentElementRepository(DatabaseService databaseService) 
    {
    ...
    public ObjectAttributeValueRow[] LoadAll() 
    {
        using SqliteDataReader sqlReader = _databaseService.Query(
            @"SELECT equipment_id, element_id, value
                FROM equipment_elements"
        );

...
public sealed class EquipmentAbilityRepository 
{
    private readonly DatabaseService _databaseService;

    public EquipmentAbilityRepository(DatabaseService databaseService) 
    {
    ...
    public AbilitySealRow[] LoadAll() 
    {
        using SqliteDataReader sqlReader = _databaseService.Query(
            @"SELECT equipment_id, ability_id, sealed
                FROM equipment_abilities"
        );
